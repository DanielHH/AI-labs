...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while (!frontier.isEmpty()) {
			SearchNode leaf_node = frontier.removeFirst();
			if (p.isGoalState(leaf_node.getState())) {
				path = leaf_node.getPathFromRoot(); // path to goal
				break;
			}
			explored.add(leaf_node);
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(leaf_node.getState());
			if(insertFront) { // DepthFirstSearch
				for (GridPos child : childStates) {
					SearchNode childNode = new SearchNode(child, leaf_node);
					if(!checkIfFrontOrExp(childNode)) {
	...
	/**
	 * Checks if param node is already in explored or frontier.
	 * @param node
	 * @return true if it already is, false otherwise
	 */
	private boolean checkIfFrontOrExp(SearchNode node) {
