...
class MyAgentProgram implements AgentProgram {
	...
	public Action execute(Percept percept) {
		...
		if (!nodesToExplore.isEmpty()) {
			...
			if (targetNode != new SimpleEntry<>(state.agent_x_position,
					state.agent_y_position)) {
				return goToTargetNode(targetNode, node_parent_list);
			}
			nodesToExplore.remove();
			...
			if (!nodesToExplore.contains(new SimpleEntry<>(
					state.agent_x_position, state.agent_y_position - 1))) { // Add
																			// northnode
				Map.Entry<Integer, Integer> nodeToVisit = new AbstractMap.SimpleEntry<>(
						state.agent_x_position, state.agent_y_position - 1);
	...
	private Action goToTargetNode(Entry<Integer, Integer> targetNode, List<Entry<Entry<Integer, Integer>, List<Entry<Integer, Integer>>>> node_parents_list) {
		List<Entry<Integer, Integer>> parent_nodes = node_parents_list.get(0).getValue();
